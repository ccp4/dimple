...
def basic_script(pdb, mtz, center, toward, white_bg, pha=None):
    assert mtz is not None or pha is not None
    same_dir = (os.path.dirname(pdb) == '' and (
        (mtz is not None and os.path.dirname(mtz) == '') or
        (pha is not None and os.path.dirname(pha) == '')))
    ...
    if mtz:
        ...
        if same_dir:
            lines += ['pha = os.path.join(this_dir, "%s")' % pha]
        else:
            lines += ['pha = "%s"' % pha]
        lines += [
            'map = read_phs_and_make_map_using_cell_symm_from_previous_mol(pha)',
